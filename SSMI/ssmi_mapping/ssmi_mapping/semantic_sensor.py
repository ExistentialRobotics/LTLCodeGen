...
class SemanticPclGenerator:
    def __init__(
        self, intrinsic, width=80, height=60, unit_conversion=1, frame_id="/camera", point_type=PointType.SEMANTIC
    ):
        """
        ...
        self.cloud_ros.fields.append(PointField(name="x", offset=0, datatype=PointField.FLOAT32, count=1))
        self.cloud_ros.fields.append(PointField(name="y", offset=4, datatype=PointField.FLOAT32, count=1))
        self.cloud_ros.fields.append(PointField(name="z", offset=8, datatype=PointField.FLOAT32, count=1))
        self.cloud_ros.fields.append(PointField(name="rgb", offset=16, datatype=PointField.FLOAT32, count=1))
        self.cloud_ros.fields.append(PointField(name="semantic_color", offset=20, datatype=PointField.FLOAT32, count=1))
    ...
    def generate_cloud_data_common(self, bgr_img, depth_img):
        ...
        """
        # rospy.loginfo(f"depth min: {np.min(depth_img)}, max: {np.max(depth_img)}")
        ...
        o_R_r = np.matrix([[0, -1, 0], [0, 0, -1], [1, 0, 0]])
        depth = depth_img.reshape(-1, 1) / self.unit_conversion
        self.xyd_vect[:, 0:2] = self.xy_index * depth
        self.xyd_vect[:, 2:3] = depth
