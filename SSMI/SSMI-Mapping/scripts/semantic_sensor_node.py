...
class SemanticCloud:
    ...
    def __init__(self):
        ...
        if point_type == 0:
            ...
            rospy.loginfo("Generate semantic point cloud.")
        else:
        ...
        self.color_sub = message_filters.Subscriber(
            ...
            Image,
            queue_size=1,
            buff_size=30 * self.img_width * self.img_height,
        )
        self.semantic_sub = message_filters.Subscriber(
            rospy.get_param("/semantic_pcl/semantic_image_topic"),
            Image,
            queue_size=1,
            buff_size=30 * self.img_width * self.img_height,
        )
        ...
        rospy.loginfo("Semantic point cloud ready!")
    ...
    def color_semantic_depth_callback(self, color_img_ros, semantic_img_ros, depth_img_ros):
        ...
        if depth_img.shape[0] != self.img_height or depth_img.shape[1] != self.img_width:
            rospy.loginfo(f"Resizing depth image from {depth_img.shape} to ({self.img_height}, {self.img_width})")
            depth_img = resize(
                depth_img,
        ...
        if semantic_img.shape[0] != self.img_height or semantic_img.shape[1] != self.img_width:
            rospy.loginfo(f"Resizing semantic image from {semantic_img.shape} to ({self.img_height}, {self.img_width})")
            semantic_img = resize(
                semantic_img,
                (self.img_height, self.img_width),
                order=0,
...
def main(args):
    ...
    try:
        ...
        rospy.loginfo("Semantic cloud shutting down!")
