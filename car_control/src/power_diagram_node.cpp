...
void PowerDiagramControlNode::onInit(void) {
  ...
  // Setup Subscribers
  odom_sub_ = n.subscribe("/tracker_odom", 10, &PowerDiagramControlNode::odom_callback, this, ros::TransportHints().tcpNoDelay());
  pose_sub_ = n.subscribe("pose", 10, &PowerDiagramControlNode::pose_callback, this, ros::TransportHints().tcpNoDelay()); // Not used
  ...
  n.param("/power_diagram_node/gains/kl", kl, 0.0);

  n.param("/power_diagram_node/vmax", vmax, 0.0);
  n.param("/power_diagram_node/wmax", wmax, 0.0);
}
