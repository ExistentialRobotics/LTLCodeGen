...
#include "solar_planner/grid_map_utils.h"
#include "solar_planner/erl_utils.h"
#include "solar_planner/boost_serialization_helper/save_load.h"
...
namespace erl
{
  ...
  public:
    ...
    //TODO: We are using this constructor
    GridMap(const std::vector<double> &mapmin,
    ...
    GridMap(const std::vector<int> &mapdim,
    ...
    bool setMapValue(const std::vector<double> &datam, const T &val)
    {
      return setMapValueCell(meters2cells(datam), val);
    }
    bool setMapValueCell(const std::vector<int> &datac, const T &val)
    {
      return inMapCell(datac) && (map_[subv2ind(datac)] = val);
    }
    bool setMapValueInd(size_t ind, const T &val)
    {
      return (ind < map_.size()) && (map_[ind] = val);
    }
    ...
    GridMap<T> getSlice(const std::vector<unsigned> &dims_to_remove,
     ...
     */
    bool intersectSegment(const std::vector<double> &start,
     ...
     */
    bool isPointVisibleFromPose(const Eigen::Vector2d &y,
  ...
  template <class T>
  std::vector<T>
  inflateMap2D(const GridMap<T> &map, double rad)
  {
    ...
    for (unsigned int x = 0; x < map.size()[0]; ++x)
      ...
      {
        if (map.map()[map.subv2ind({(int)x, (int)y})] > 0)
        { // If the cell is occupied,
          // Set its neighbors to 1.
          for (const auto &it : ns)
          {
            int nx = x + it[0], ny = y + it[1];
            if (0 <= nx && nx < map.size()[0] && 0 <= ny && ny < map.size()[1])
              omap[map.subv2ind({nx, ny})] = 1;
          }
        }
      }
    return omap;
...
{
  std::cout << "this is the map size: " << map.size() << std::endl;
...
inline void erl::GridMap<T>::save(const std::string &filename) const
{
  ...
  out << YAML::BeginSeq;
  for (size_t k = 0; k < min_.size(); ++k)
    out << min_[k];
  out << YAML::EndSeq;
  out << YAML::Key << "mapmax";
  out << YAML::Value << YAML::Flow;
  out << YAML::BeginSeq;
  for (size_t k = 0; k < max_.size(); ++k)
    out << max_[k];
  out << YAML::EndSeq;
  out << YAML::Key << "mapres";
  out << YAML::Value << YAML::Flow;
  out << YAML::BeginSeq;
  for (size_t k = 0; k < res_.size(); ++k)
    out << res_[k];
  out << YAML::EndSeq;
  out << YAML::Key << "mapdim";
  out << YAML::Value << YAML::Flow;
  ...
  out << YAML::Value << YAML::Flow;
  out << YAML::BeginSeq;
  for (size_t k = 0; k < origin_.size(); ++k)
    out << origin_[k];
  out << YAML::EndSeq;
  out << YAML::Key << "origincells";
  out << YAML::Value << YAML::Flow;
  out << YAML::BeginSeq;
  for (size_t k = 0; k < origincells_.size(); ++k)
    out << origincells_[k];
  out << YAML::EndSeq;
...
inline bool erl::GridMap<T>::load(const std::string &filename)
{
  ...
  {
    initMapData2(map_spec["mapdim"].as<std::vector<int>>().size(),
                 map_spec["mapdim"].as<std::vector<int>>().data(),
  ...
  else if (map_spec["mapmin"] && map_spec["mapmax"] && map_spec["mapres"] && map_spec["storage"] && map_spec["mappath"])
  {
    initMapData1(map_spec["mapmin"].as<std::vector<double>>().size(),
                 map_spec["mapmin"].as<std::vector<double>>().data(),
                 map_spec["mapmax"].as<std::vector<double>>().data(),
                 map_spec["mapres"].as<std::vector<double>>().data(),
                 map_spec["storage"].as<std::string>() == "rowmajor" ? true : false);
    serialization::loadBin(map_, parentpath + "/" + map_spec["mappath"].as<std::string>(), true);
    return true;
  ...
  if (map_spec["mapdim"] && map_spec["origin"] && map_spec["mapres"] && map_spec["storage"] && map_spec["mappath"])
  {
    ...
    if (size_.size() > 0)
    {
      ...
      for (unsigned k = 0; k < size_.size(); ++k)
        sz *= size_[k];
      map_.resize(sz);
      for (unsigned k = 0; k < map_.size(); ++k)
        fsr >> map_[k];
      fsr.close();
    }
    return true;
...
inline void erl::GridMap<T>::toColmajor()
{
  ...
  for (size_t k = 0; k < mapsz; ++k)
    newmap[subv2ind_colmajor(ind2subv_rowmajor(k))] = map_[k];
  map_ = newmap;
}
...
inline void erl::GridMap<T>::toRowmajor()
{
  ...
  std::vector<T> newmap(mapsz);
  for (size_t k = 0; k < mapsz; ++k)
    newmap[subv2ind_rowmajor(ind2subv_colmajor(k))] = map_[k];
  map_ = newmap;
}
...
template <typename T>
inline void erl::GridMap<T>::initMapData1(unsigned numdim,
                                          const double *mapmin,
                                          const double *mapmax,
                                          const double *mapres,
                                          bool rowmajor)
{
  ...
  row_major_ = rowmajor;
  // print our size_ shape and data
  std::cout << "size_: ";
  for (auto i : size_)
    std::cout << i << " ";
}
...
template <typename T>
inline void erl::GridMap<T>::initMapData2(unsigned numdim,
                                          const int *mapdim,
                                          const double *origin,
                                          const double *mapres,
                                          bool rowmajor)
...
inline erl::GridMap<T> erl::GridMap<T>::get2DSlice(size_t z_dim) const
{
  ...
  {
    erl::GridMap<T> slice(std::vector<int>({size_[0], size_[1]}),
                          std::vector<double>({origin_[0], origin_[1]}),
                          std::vector<double>({res_[0], res_[1]}), row_major_);
    std::vector<T> slice_map(size_[0] * size_[1]);

    if (!row_major_)
    { // For Column-major order
      std::copy(map_.begin() + z_dim * slice_map.size(),
                map_.begin() + (z_dim + 1) * slice_map.size(),
                slice_map.begin());
    }
    else
    { // For Row-major order
...
template <typename T>
inline bool erl::GridMap<T>::intersectSegment(
...
template <typename T>
inline bool erl::GridMap<T>::isPointVisibleFromPose(const Eigen::Vector2d &y,
                                                    const Eigen::Matrix2d &R,
                                                    const Eigen::Vector2d &p,
                                                    const Eigen::Vector2d &range_bracket,
                                                    const Eigen::Vector2d &azimuth_bracket)
{
  ...
  if (intersectSegment(p.data(), y.data(), intersection_point))
    return false;
...
template <typename T>
inline bool erl::GridMap<T>::isPointVisibleFromPose(const Eigen::Vector3d &y,
                                                    const Eigen::Matrix3d &R,
                                                    const Eigen::Vector3d &p,
                                                    const Eigen::Vector2d &range_bracket,
                                                    const Eigen::Vector2d &azimuth_bracket,
